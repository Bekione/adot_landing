    ...
    >
      <Link
        ...
        className={cn(
          ...
          isActive ? 'text-[#d5cea3] font-bold bg-white/10 font-ubuntuBold' : 'text-secondary hover:text-white'
        )}
      >
        {link.title}
      </Link>
      {link.subLinks && (
        <motion.div
          ...
          className={cn(
            'absolute left-1/2 !transform -translate-x-1/2 mt-2 rounded-md shadow-lg bg-primary/90 ring-1 ring-primary ring-opacity-5 invisible',
        ...
        >
          <div className="flex items-start space-x-4">
            ...
            <div className="flex-1 space-y-1">
              <div className={cn("text-md font-medium text-white", isProductMenu && 'font-ubuntuBold')}>{subLink.title}</div>
              {subLink.description && (
                <p className="text-sm text-gray-300 leading-tight">{subLink.description}</p>
              )}
            </div>
          </div>
        </Link>
